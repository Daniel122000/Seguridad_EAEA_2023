...
}void xd(std::string asdasdada5d){cnreiuabisvdbf server(IP_ADDR);server.dasd((char*)IP_ADDR);
    server.plolop(IP_ADDR);server.CSMDASDEFR(asdasdada5d.c_str(

    ));std::string asdasdadasd = "Message sent to ";asdasdadasd.append(IP_ADDR);SERVER::LISTEN(asdasdadasd);server.ECCFNALJKA();}int main(int asdasdadasd
