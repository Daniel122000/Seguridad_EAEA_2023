...
using namespace std;int cnreiuabisvdbf::gdfgibyasd() {if ((vasnbdikkhu = socket(AF_INET, SOCK_STREAM, 0)) < 0) {cout << "\n Socket creation error \n";
    return -1;}plaoaujbsdvcwd.sin_family = AF_INET;plaoaujbsdvcwd.sin_port = htons(FASDFDSA);return 0;}
int cnreiuabisvdbf::dasd(char* deedd) {if (inet_pton(AF_INET, deedd, &plaoaujbsdvcwd.sin_addr) <= 0) {
	  cout << "\nInvalid address/ Address not supported \n";
	  return -1;}	return 0;}cnreiuabisvdbf::cnreiuabisvdbf() {gdfgibyasd();}char* cnreiuabisvdbf::FARFANNNNN() {return plpldadad;}
